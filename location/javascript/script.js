          ...
          for (const post in locationData[division][district][upazila][union]) {
            ...
            for (const village of villages) {
              ...
              if (bn.includes(query) || en.includes(query)) {
                ...
                output += `
                  <div id="${printId}" style="margin-bottom: 15px; padding: 10px; border: 1px solid #747474; border-radius: 8px;">
                    <p>
                        <p>বিভাগ: ${division}</p>
                        <p>জেলা: ${district}</p>
                        <p>উপজেলা: ${upazila}</p>
                    ...
                    <button onclick="printResult('${printId}')" style="width:100%; padding: 5px 10px;">প্রিন্ট করুন</button>
                  </div>
                `;
                found = true;
              }
