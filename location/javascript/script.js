...
function searchLocation() {
  ...
  const mapFrame = document.getElementById("mapFrame"); // ✅ ম্যাপ এলিমেন্ট ধরো

  if (query === "") {
    resultDiv.innerHTML = "";
    mapFrame.src = "";             // ✅ ম্যাপ লিংক খালি
    mapFrame.style.display = "none"; // ✅ ম্যাপ লুকাও
    return;
  }
          ...
          for (const post in locationData[division][district][upazila][union]) {
            ...
            for (const village of villages) {
              ...
              if (bn.includes(query) || en.includes(query)) {
                ...
                output += `
                  <div id="${printId}" style="margin-bottom: 15px; border: 2px solid var(--shadow-dark); padding: 10px; border-radius: 8px;">
                    <p>
                      <p>বিভাগ: ${division}</p>
                      <p>জেলা: ${district}</p>
                      <p>উপজেলা: ${upazila}</p>
                    ...
                    <button onclick="printResult('${printId}')" class="printbtn" {
                      constructor() {
                        this.prop = ""
                      }
                    }>প্রিন্ট করুন</button>
  ...
  resultDiv.innerHTML = found ? output : "কিছু খুঁজে পাওয়া যায়নি ⚠️";
